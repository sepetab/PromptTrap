...
st.set_page_config(page_title="PurifyDocs", page_icon=":shield:", layout="wide")
...
def _build_summary_markdown(filename: str, result: Any) -> str:
    ...
    lines.append("---")
    lines.append("")
    lines.append("*Generated by PurifyDocs — local-first document-safety scanner.*")
    ...
    for report_path in reports[:10]:
        ...
        with col_btn:
            if st.button("View", key=f"view_{name}"):
                ...
                from purifydocs.scanner.base import Issue, ScanResult
...
PurifyDocs scans documents for hidden or machine-readable manipulation that could
...
- **No accounts, no tracking** — PurifyDocs does not require any login and does
...
docker compose run --rm app python -m purifydocs benchmark run --input data/generated --out reports/benchmark.json
```
...
def main() -> None:
    ...
    st.sidebar.divider()
    st.sidebar.markdown("---")
    st.sidebar.markdown("### Support PurifyDocs")
    st.sidebar.markdown(
        "If you find PurifyDocs useful, consider supporting development:"
    )
    st.sidebar.markdown(
        '[![Buy Me A Coffee](https://img.shields.io/badge/Buy%20Me%20A%20Coffee-%E2%98%95-yellow)]'
        '(https://www.buymeacoffee.com/purifydocs)',
        unsafe_allow_html=True,
    )
